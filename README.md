...
Untrusted input is a document; it can only ever become one of a fixed set of filing commands, never a tool argument. An injected instruction cannot be represented in the closed type, so it is rejected — proven at compile time by `check`, and clamped again at run time by the `extract` domain check.

- **Untrusted input:** `fetch<web>` — a document
- **Closed type:** `type Category = Invoice | Contract | Report | Resume`, then `type Decision = Classify(Category) | Route(Dept) | Reject` (payload-carrying variants; the payloads are closed too)
- **Sink / capability:** `grant file` → `file(d)`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/document-classifier/demo.sh
```
...
- `document-classifier_safe.wave` — the correct design.
- `document-classifier_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
